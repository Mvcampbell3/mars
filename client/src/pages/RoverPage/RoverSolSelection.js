...
const RoverSolSelection = (props) => {
    ...
    useEffect(() => {
        const correctSol = [...manifestSols].filter(
            (manSol) => manSol.sol === Number(selectedSol),
        );
        setSelectedCamera("");
    ...
    const InputProps = useCallback(
        (changeFunc, value, name, id, placeholder) => {
            return {
                handleChange: (e) => {
                    ...
                    <div className="sol-input-group">
                        ...
                        <NumberInput
                            {...InputProps(
                                ...
                                "selected-sol-input",
                                "selected-sol-input",
                                "A number less or equal to the max sol number",
                            )}
                        />
                        <h1>{selectedSol}</h1>
                    </div>
